...
import {BUNDLE_HANDLE, getProductDescription, getBundleValueProposition} from '~/content/products';
import {cn} from '~/utils/cn';
import {formatMoney} from '~/utils/format-money';
import type {MoneyFragment, RecommendedProductFragment} from 'storefrontapi.generated';
 ...
 */
export const ProductRevealInfo = React.forwardRef<HTMLDivElement, ProductRevealInfoProps>(
  ({product, variantPrice, className}, ref) => {
    ...
    const bundleValueProp = isBundle
      ? getBundleValueProposition(product.handle, product.bundleValueProposition?.value)
      : null;

    // Get description with fallback (AC3)
    const description = getProductDescription(product.handle, product.description);
    ...
    return (
      <div
        ref={ref}
        className={cn('flex flex-col gap-3 p-4', 'bg-gradient-to-t from-black/60 to-transparent', className)}
      >
        {/* Product name with fluid-heading typography (AC1) */}
        <h3 className="text-fluid-heading text-white font-semibold">{product.title}</h3>

        {/* Bundle value proposition (Story 3.6, Task 4) */}
        {bundleValueProp && <p className="text-fluid-body text-white/90 italic">{bundleValueProp}</p>}
        ...
        {truncatedDescription && (
          <p className="text-fluid-body text-white/90 leading-relaxed">{truncatedDescription}</p>
        )}
        ...
        {isBundle ? (
          <div className="mt-2">
            {variantId ? (
              ...
              <div className="text-fluid-body text-white/90 text-center" role="alert">
                Product unavailable
              </div>
            )}
          </div>
